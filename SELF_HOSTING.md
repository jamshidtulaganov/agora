...
Deploy Agora on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`agora` CLI** and runs the **agent daemon** on their own machine.
...
This installs the `agora` CLI, checks out the latest self-host assets, pulls the official Agora images from GHCR, and configures everything for localhost.
...
>
> **CLI only?** If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it from Agora's own GitHub Release:
>
> ```bash
...
`make selfhost-build` uses local `agora-backend:dev` / `agora-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `AGORA_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `AGORA_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
If you already run a Kubernetes cluster, you can deploy Agora there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/agora-ai/charts/agora` or the source chart at [`deploy/helm/agora/`](deploy/helm/agora/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `agora-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `agora-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `agora-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `agora-config` ConfigMap (rendered from `values.yaml`)

The `agora-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **One release per namespace:** the prebuilt `agora-web` image bakes `REMOTE_API_URL=http://backend:8080` at build time, so the chart ships an ExternalName Service literally named `backend`. Because that name is unprefixed, you can run only one Agora release per namespace, and `helm install` will fail if a `Service/backend` already exists there (pass `--take-ownership`, or use a dedicated namespace). If you build a web image with a patched `REMOTE_API_URL`, set `frontend.compatibility.backendAlias: false` to drop the alias.
...
The chart defaults to `agora.dev.lan` (web) and `api.agora.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  agora.dev.lan api.agora.dev.lan
  ```
...
### Step 3 — Create the `agora-secrets` Secret
...
kubectl -n agora create secret generic agora-secrets \
  ...
  --from-literal=AGORA_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install agora oci://ghcr.io/agora-ai/charts/agora \
...
```bash
helm show values oci://ghcr.io/agora-ai/charts/agora \
...
```bash
helm install agora deploy/helm/agora -n agora
```

Watch the pods come up:

```bash
kubectl -n agora get pods -w
```
...
```bash
curl -H "Host: api.agora.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://agora.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n agora patch secret agora-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n agora rollout restart deploy/agora-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n agora logs -f deploy/agora-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `AGORA_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade agora oci://ghcr.io/agora-ai/charts/agora \
    --version <chart-version> \
    -n agora \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n agora patch secret agora-secrets --type=merge \
    -p '{"stringData":{"AGORA_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n agora rollout restart deploy/agora-backend
  ```
...
```bash
agora setup self-host \
  --server-url http://api.agora.dev.lan \
  --app-url http://agora.dev.lan
```
...
```bash
kubectl -n agora rollout restart deploy/agora-backend deploy/agora-frontend
```

To upgrade to a specific Agora release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
helm -n agora rollback agora
```

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** As of MUL-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically before applying migration `103`, so a clean upgrade is a single `helm upgrade` + backend rollout. If you are still on a pre-MUL-2957 binary or the auto-hook fails, run the standalone backfill against the same database the chart is using (`kubectl -n agora exec deploy/agora-backend -- ./backfill_task_usage_hourly --sleep-between-slices=2s`), then restart the backend deployment to re-apply migrations. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the full recovery flow.

### Tearing down

```bash
# Remove the workloads but keep the PVCs and the Secret
helm -n agora uninstall agora
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Agora's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
```bash
curl -fsSL https://raw.githubusercontent.com/jamshidtulaganov/agora-cli/main/install.sh | bash -s -- --stop
```
...
## Switching to Agora Cloud

If you've been self-hosting and want to switch your CLI to [Agora Cloud](https://agora.dev):

```bash
agora setup
```

This reconfigures the CLI for agora.dev, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `AGORA_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `agora setup`:
