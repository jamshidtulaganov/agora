...
This document is designed for AI agents to execute. Follow these steps exactly to deploy a local Tandem instance and connect to it.
...
Wait for the server output `✓ Tandem server is running and CLI is ready!` before running `tandem setup self-host`.

**Expected result:**
- Frontend at http://localhost:3000
- Backend at http://localhost:8080
- `tandem` CLI installed and configured for localhost
...
The `tandem setup self-host` command will:
...
2. Run `make selfhost`
3. Run `tandem setup self-host --port <PORT> --frontend-port <FRONTEND_PORT>`

## Troubleshooting

- **Backend not ready:** `docker compose -f docker-compose.selfhost.yml logs backend`
- **Frontend not ready:** `docker compose -f docker-compose.selfhost.yml logs frontend`
- **Daemon issues:** `tandem daemon logs`
