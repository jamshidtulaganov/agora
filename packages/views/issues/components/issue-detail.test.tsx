...
describe("IssueDetail (shared)", () => {
  ...
  it("marks unseen comments with a compact indicator without changing card spacing", async () => {
    ...
    expect(wrapper).toHaveClass("relative", "pb-2");
  ...
  it("truncates the trailing activity block to the most recent 3 entries with a show-more toggle", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Show 7 more activities")).toBeInTheDocument();
    });
    expect(screen.queryByText("10 activities")).not.toBeInTheDocument();

    // Only the 3 most recent entries (act-8..act-10) are rendered by default.
    // The older activity log is folded behind the show-more line.
    expect(screen.getByText(/from Urgent to Low/i)).toBeInTheDocument(); // act-8
    ...
    expect(screen.queryByText(/from In Progress to In Review/i)).not.toBeInTheDocument(); // act-3
    ...
    fireEvent.click(screen.getByText("Show 7 more activities"));
    await waitFor(() => {
  ...
  it("does not show the show-more toggle when the trailing block has 3 or fewer entries", async () => {
    const trailingBlock: TimelineEntry[] = [
      { type: "activity", id: "act-1", actor_type: "member", actor_id: "user-1", action: "status_changed", details: { from: "todo", to: "in_progress" }, created_at: "2026-01-18T00:00:00Z" },
    ...
    // Every one of the 3 entries should be visible — the trailing block fits
  ...
  it("expanding a non-trailing block shows every entry — only the trailing block truncates older ones", async () => {
    ...
    // truncate-to-3 rule applies only to the trailing block.
    ...
    // block has more than 3 entries, the truncate-to-3 rule does not apply
