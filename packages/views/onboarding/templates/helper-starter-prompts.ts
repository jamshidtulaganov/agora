 ...
 * Agora Helper is created. Each card maps to ONE issue being created
...
- 5–8 slides total:
  1. Title — "What Agora can do for [my role]"
  2. Four core concepts — workspace / issue / agent / runtime, one slide
  3–6. 3–4 concrete scenarios tailored to my use case, each in the form "When you want X → here's how Agora handles it"
...
When done, also reply with a one-sentence summary of which scenarios you picked for me and why.`,
      zh: `给我做一份单文件 HTML 演示稿,介绍 Agora 能为我做什么。根据我的角色和使用场景定制(见下面"关于我")。把完整 HTML 贴到这条 issue 的评论里的 \`\`\`html 代码块中,我直接复制下来存成 \`agora-intro.html\` 双击就能在浏览器里打开。

**产出格式**
- 一个自包含 .html,CSS / JS 全部 inline。零依赖、不用打包、不引外部图片(视觉用纯 CSS 生成 —— 渐变、几何形状、内联 SVG)。
- 5-8 张 slide:
  1. 标题页 —— "Agora 能为 [我的角色] 做什么"
  2. 四个核心概念 —— workspace / issue / agent / runtime,一张
  3-6. 3-4 个针对我使用场景的具体例子,形如"当你想做 X → Agora 是这样处理的"
  7. 收尾页 —— 一个具体的下一步动作
