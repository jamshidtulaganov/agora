...
import { larkInstallationsOptions, larkKeys } from "@agora/core/lark";
import { api, ApiError } from "@agora/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@agora/core/types";
...
// install path is per-agent (each Agora Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Agora Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Agora agent's
