...
import { larkInstallationsOptions, larkKeys } from "@tandem/core/lark";
import { api, ApiError } from "@tandem/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@tandem/core/types";
...
// install path is per-agent (each Tandem Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Tandem Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Tandem agent's
