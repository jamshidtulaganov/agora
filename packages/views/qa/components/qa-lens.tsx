...
export function QALensBody({ issueId }: { issueId: string }) {
  ...
  // human hasn't decided yet. A human click always writes the real qa:pass/
  // qa:fail label and takes over (humanVerdict, once set, always wins).
          ...
          <div className="order-1 flex min-w-0 flex-col lg:order-2 lg:sticky lg:top-8 lg:h-[calc(100vh-10rem)] lg:min-h-0">
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-y-auto pr-0.5 max-lg:max-h-[65vh]">
              ...
              {/* Verdict — compact: one line (icon + label + captured time), with
                  the summary clamped to two lines (full text on hover). */}
                ...
                <div className={cn("rounded-lg border px-3 py-2", verdictTone(verdict))}>
                  <div className="flex items-center gap-2">
                    ...
                    {/* Provenance: who produced this verdict — a real agent run vs
                        ...
                        regression. "" from older rows = agent. */}
                    {evidence && (
            ...
            {/* Triage bar — the QA team's verdict + actions. Pinned to the
                bottom of the column (flex, shrink-0) so the pass/fail call is
