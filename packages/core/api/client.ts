...
export class ApiClient {
  ...
  private readCsrfToken(): string | null {
    ...
    const match = document.cookie
      .split("; ")
      .find((c) => c.startsWith("tandem_csrf="));
  ...
  // ---------------------------------------------------------------------
  // Cloud Billing — proxies to tandem-cloud /api/v1/billing/*. The
  // tandem-api server stamps X-User-ID and forwards bytes; everything
