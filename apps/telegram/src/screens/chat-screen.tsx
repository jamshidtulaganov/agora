...
export function ChatScreen() {
  ...
  const agentName = (id: string) => agents.find((a) => a.id === id)?.name ?? t("common.agent");
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-2.5">
        ...
        <button
          ...
          disabled={!hasAgents}
          className="flex items-center gap-1 rounded-full bg-brand px-3 py-1.5 text-xs font-semibold text-brand-foreground disabled:opacity-40"
        >
      ...
      {isLoading ? (
        <CenterMessage spinner title={t("chat.loading")} />
      ) : !hasAgents ? (
        <CenterMessage
          icon={<Bot className="size-7 text-muted-foreground" />}
          title={t("chat.noAgents")}
          subtitle={t("chat.noAgentsSub")}
        />
        ...
        <ul className="flex-1 divide-y divide-border overflow-y-auto">
          {activeSessions.map((s) => (
            <li key={s.id}>
              ...
              >
                <Avatar name={agentName(s.agent_id)} isAgent size={38} />
                ...
                {s.has_unread && <span className="size-2 shrink-0 rounded-full bg-brand" />}
      ...
      <BottomSheet open={picking} onClose={() => setPicking(false)} title={t("chat.startChat")}>
        <ul className="pb-2">
          {activeAgents.map((a) => (
            <li key={a.id}>
              <button
                ...
                className={cn(
                  "flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm active:bg-accent",
                  createSession.isPending && "opacity-50",
                )}
              >
                <Avatar name={a.name} isAgent size={32} />
