...
// agora CLI can find agent binaries like claude/codex/opencode. Must run
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      plugins: true,
      additionalArguments: [`--agora-locale=${systemLocale}`],
    },
  });
...
// single-worktree case still lands at "Agora Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Agora Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Agora Canary";
...
if (is.dev) {
  ...
  // to "Agora", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.agora.desktop.dev" : "ai.agora.desktop",
    );
