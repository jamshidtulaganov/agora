...
// tandem CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // tandem://auth/callback?token=<jwt>
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      plugins: true,
      additionalArguments: [`--tandem-locale=${systemLocale}`],
    },
  });
...
// single-worktree case still lands at "Tandem Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Tandem Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Tandem Canary";
...
if (is.dev) {
  ...
  // to "Tandem", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.tandem.desktop.dev" : "ai.tandem.desktop",
    );
