...
const PREFS_PATH = join(homedir(), ".agora", "desktop_prefs.json");
...
// `agora daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/agora/cmd_daemon.go) to
...
// Keep the Go impl in sync: server/cmd/agora/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".agora", "profiles", profile)
    : join(homedir(), ".agora");
}
...
// Sidecar file that records which Agora user the cached PAT in config.json
 ...
 * a stuck start is an auth problem. Hits the same endpoint `agora auth status`
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["agora.exe"] : ["agora"];
 ...
 *   to `apps/desktop/resources/bin/agora`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Agora.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "agora.exe" : "agora";
...
/**
 * Returns a usable `agora` binary path. Priority:
 ...
 *   5. `agora` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Agora Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, AGORA_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "agora CLI is not installed" };
