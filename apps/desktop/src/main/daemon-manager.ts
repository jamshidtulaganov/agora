...
const PREFS_PATH = join(homedir(), ".tandem", "desktop_prefs.json");
...
// `tandem daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/tandem/cmd_daemon.go) to
...
// Keep the Go impl in sync: server/cmd/tandem/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".tandem", "profiles", profile)
    : join(homedir(), ".tandem");
}
...
// Sidecar file that records which Tandem user the cached PAT in config.json
 ...
 * a stuck start is an auth problem. Hits the same endpoint `tandem auth status`
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["tandem.exe"] : ["tandem"];
 ...
 *   to `apps/desktop/resources/bin/tandem`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Tandem.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "tandem.exe" : "tandem";
...
/**
 * Returns a usable `tandem` binary path. Priority:
 ...
 *   5. `tandem` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Tandem Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, TANDEM_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "tandem CLI is not installed" };
