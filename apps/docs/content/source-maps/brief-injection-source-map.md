...
| --- | --- | --- |
| Marker constants; changing them is a breaking change; legacy MULTICA-RUNTIME pair still recognised after the rebrand rename | `runtimeMarkerBegin`, `runtimeMarkerEnd`, `legacyRuntimeMarkerBegin`, `legacyRuntimeMarkerEnd` (const block ~L33-43) | `"<!-- BEGIN AGORA-RUNTIME (auto-managed; do not edit) -->"`, `"<!-- END AGORA-RUNTIME -->"`; legacy `"<!-- BEGIN MULTICA-RUNTIME (auto-managed; do not edit) -->"`, `"<!-- END MULTICA-RUNTIME -->"` |
...
| Idempotent replace not overwrite; MUL-2753; three file states; block format; unconditional separator | `writeRuntimeConfigFile` (~L225-253) | `block := runtimeMarkerBegin + "\n" + strings.TrimRight(brief, "\n") + "\n" + runtimeMarkerEnd + "\n"`; `errors.Is(err, fs.ErrNotExist)` → write block only; replace branch `existingStr[:start] + block + existingStr[end:]`; append branch `existingStr+runtimeManagedSeparator+block` |
| END strictly after BEGIN; stray-END and crash-half-block cases; BEGIN-no-END = block-to-EOF; end consumes trailing newline; current markers tried first then legacy pair | `locateMarkerBlock` + `locateMarkerBlockWith` (~L279-309) | `if start, end, ok := locateMarkerBlockWith(content, runtimeMarkerBegin, runtimeMarkerEnd); ok { return ... }` then legacy pair; `endRel := strings.Index(content[afterBegin:], end)`; `if endRel < 0 { return start, len(content), true }`; `if blockEnd < len(content) && content[blockEnd] == '\n' { blockEnd++ }` |
| Byte-exact rollback; three states; created-file via missing separator → os.Remove; pre-existing → write remainder verbatim; no normalisation | `CleanupRuntimeConfig` (~L338-383) | `hadManagedSeparator := strings.HasSuffix(pre, runtimeManagedSeparator)`; `if !hadManagedSeparator && remainder == "" { os.Remove(path) }`; else `os.WriteFile(path, []byte(remainder), 0o644)` |
