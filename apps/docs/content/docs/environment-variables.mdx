...
Zoho is set up in the product, not in environment variables:

1. A workspace owner or admin opens **Settings → Integrations → Zoho** and sets up the **Zoho connector**: in api-console.zoho.com they create a *Server-based application*, register the redirect URI the card shows (`<AGORA_PUBLIC_URL>/api/integrations/zoho/callback`), and paste its Client ID, Client secret and data center. A refresh token is optional — it is only needed to sync Zoho CRM records into issues.
2. Each person then clicks **Connect Zoho** in the **Your Zoho account** card just below it and signs in to Zoho as themselves. Every Zoho read the Assistant or an agent makes for them uses that person's grant, so Zoho's own role, profile and sharing rules decide what Agora sees. The grant only carries read scopes; Agora never creates or changes anything in Zoho.

The server itself needs only:

| Variable | Default | Description |
|---|---|---|
| `AGORA_ZOHO_SECRET_KEY` | empty | 32-byte base64 key that seals connector secrets and each person's grant at rest. Without it, the connector can't be saved |
| `AGORA_PUBLIC_URL` | empty | The redirect URI is built from it, never from request headers. Without it, nobody can connect |

**Local development:** `go run ./cmd/fakezoho` serves an in-memory Zoho with two people who see different deals and tickets. Point the backend at it with `ZOHO_DYN_ACCOUNTS_BASE` / `ZOHO_DYN_API_BASE` / `ZOHO_DYN_DESK_BASE`, then set up the connector with Client ID `1000.FAKEZOHOCLIENT` and Client secret `fake-zoho-secret`.
