...
description: The full list of environment variables for running a self-hosted Agora server.
---

import { Callout } from "fumadocs-ui/components/callout";

A self-hosted Agora [server](/self-host-quickstart) reads its configuration from environment variables at startup — database, sign-in, email, storage, signup allowlists all live here. This page groups every variable by purpose: each section spells out **what happens if you leave it unset** and **which ones you must set in production**. For how to actually configure the auth-related ones, see [Sign-in and signup configuration](/auth-setup).
...
|---|---|---|
| `DATABASE_URL` | `postgres://agora:agora@localhost:5432/agora?sslmode=disable` | **Yes** |
| `PORT` | `8080` | No (unless you change the port) |
| `JWT_SECRET` | `agora-dev-secret-change-in-production` | **Yes** (the default is unsafe) |
| `APP_ENV` | empty | **Yes** (must be `production`) |
| `FRONTEND_ORIGIN` | empty | **Yes** (self-host must set its own domain) |
| `AGORA_DEV_VERIFICATION_CODE` | empty | No (must stay empty in production) |

<Callout type="warning">
**Keep `AGORA_DEV_VERIFICATION_CODE` empty in production.** A fixed local test code is disabled by default, but if you opt in with `AGORA_DEV_VERIFICATION_CODE=888888`, anyone who can request a code can sign in with that fixed value while `APP_ENV` is non-production. The shortcut is ignored when `APP_ENV=production`.
</Callout>
...
Agora supports two delivery backends — [Resend](https://resend.com/) for cloud deployments, or an SMTP relay for internal / on-premise networks. `SMTP_HOST` takes priority over `RESEND_API_KEY` when both are set.
...
| `RESEND_FROM_EMAIL` | `noreply@agora.dev` | Sender address (must be a domain verified in your Resend account; also reused as the `From:` header when SMTP is in use) |
...
Agora stores user-uploaded attachments (images and files in comments). **S3 is preferred**; if S3 is not configured, it falls back to local disk.
...
**Invite flows themselves do not check the signup allowlist** — but the invitee must still be able to **sign in** before accepting the invite. If they already have a Agora account (for example from another workspace), they can accept directly, unaffected by the allowlist; **if they have never signed up**, the first step of sign-in (requesting a verification code) still passes through the allowlist check, and an email rejected by `ALLOW_SIGNUP=false` or by `ALLOWED_EMAILS` / `ALLOWED_EMAIL_DOMAINS` **cannot finish signup, and therefore cannot accept the invite**.
...
This separate `RATE_LIMIT_TRUSTED_PROXIES` is **not** the same as `AGORA_TRUSTED_PROXIES`, which controls the autopilot-webhook limiter (`/api/webhooks/autopilots/{token}`). Each limiter parses its own list, so a deployment behind a proxy should set both.
...
| Variable | Default | Description |
|---|---|---|
| `AGORA_SERVER_URL` | `ws://localhost:8080/ws` | Server address (self-host: replace with your domain) |
| `AGORA_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `AGORA_DAEMON_POLL_INTERVAL` | `3s` | Task polling interval |
| `AGORA_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Max concurrent tasks |
| `AGORA_<PROVIDER>_PATH` | matches the CLI name | Path to each AI coding tool's executable (for example `AGORA_CLAUDE_PATH`) |
| `AGORA_<PROVIDER>_MODEL` | empty | Default model for each AI coding tool |
...
| Variable | Default | Description |
|---|---|---|
| `FRONTEND_ORIGIN` | empty | Frontend address. Invite email links, the CORS allowlist, and the cookie domain are all derived from this. When unset, invite email links fall back to the hosted domain `https://app.agora.dev` — self-host must set this explicitly |
| `AGORA_APP_URL` | empty | Frontend URL for CLI login flow. Also used by the web UI to show self-host daemon setup commands with your app domain; for same-origin deployments this is also used as daemon `server_url` when `AGORA_PUBLIC_URL` is unset |
| `AGORA_PUBLIC_URL` | empty | Public API URL, without trailing slash. Used for autopilot webhook URLs and by the web UI as the daemon `server_url` |
...
- The `/api/webhooks/github` endpoint returns **`503 github webhooks not configured`** — Agora refuses to process events with no secret rather than treating every signature as valid.

**Behavior when the optional `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY` are unset:**

- The connection card briefly shows `Connected to unknown` after install. Agora refreshes the row to the real org/user name as soon as GitHub delivers the `installation.created` webhook (typically within a few seconds), and broadcasts a realtime update so any open Settings → GitHub tab reflects the change without a manual refresh.
...
The [Slack app](/slack-integration) — channel notifications, inbox DMs and link unfurls — needs four variables. All four or nothing: a half-configured deployment hides the Connect button rather than offering one that dies after an admin has already granted scopes in Slack.

| Variable | Default | Description |
|---|---|---|
| `AGORA_SLACK_CLIENT_ID` | empty | Client ID from your Slack app's Basic Information page. Used to build the OAuth consent URL |
| `AGORA_SLACK_CLIENT_SECRET` | empty | Client secret. Sent in the `oauth.v2.access` body to exchange the single-use code for a bot token |
| `AGORA_SLACK_SIGNING_SECRET` | empty | Signing secret. Every delivery to `/slack/events` is verified with HMAC-SHA256 over `v0:{timestamp}:{raw body}` inside a five-minute window |
| `AGORA_SLACK_SECRET_KEY` | empty | 32-byte base64 key that seals each workspace's bot token at rest. Storing a token in plaintext is not an accepted fallback, so an install refuses to start without it |
| `AGORA_SLACK_APP_ID` | empty | Optional. When set, event deliveries claiming a different `api_app_id` are rejected — defence in depth against a mis-pointed subscription |
| `AGORA_SLACK_NOTIFY_ENABLED` | `true` | Project-scoped switch for the whole outbound fanout, so one noisy project can be silenced without uninstalling |

`AGORA_PUBLIC_URL` must also be set: the OAuth redirect (`<public url>/slack/oauth/callback`) is built from it and never from request headers, so a misconfigured proxy cannot talk the server into minting a redirect at an attacker's host.

**Behavior when any of the four required variables is unset:**

- Settings → Integrations does not show the Slack card at all (`/api/config` reports `slack_enabled: false`).
- `POST /api/workspaces/{id}/slack/install/begin` returns **503**.
- `/slack/events` returns **503** — without the signing secret a forged delivery is indistinguishable from a real one, and "accept everything" is not a fallback.

**Note:** these variables are for the Slack *app*. The Slack **Incoming Webhook** release connector (Settings → Integrations → Release) needs none of them — it stores its webhook URL per workspace and keeps `release:shipped` / `deploy:recorded` for itself.

## Zoho (per-person, read-only)

Each person connects their **own** Zoho account once (Settings → Profile → Connected accounts). Every Zoho read the Assistant or an agent makes for them uses that person's grant, so Zoho's own role, profile and sharing rules decide what Agora sees — nobody gets more than they can see in Zoho. The grant only carries read scopes; Agora never creates or changes anything in Zoho.

| Variable | Default | Description |
|---|---|---|
| `AGORA_ZOHO_CLIENT_ID` | empty | Client ID of a **server-based** client registered in api-console.zoho.com. Register the redirect URI `<AGORA_PUBLIC_URL>/api/integrations/zoho/callback` and enable multi-DC if people are in more than one Zoho data center |
| `AGORA_ZOHO_CLIENT_SECRET` | empty | That client's secret |
| `AGORA_ZOHO_SECRET_KEY` | empty | 32-byte base64 key that seals each person's grant at rest (shared with the workspace Zoho connection). Without it, Connect Zoho stays hidden |
| `AGORA_ZOHO_SCOPES` | read-only CRM + Desk list | Optional override of the requested scopes. The default is `ZohoCRM.modules.READ`, `ZohoCRM.coql.READ`, `ZohoCRM.settings.READ`, `ZohoCRM.users.READ`, `Desk.tickets.READ`, `Desk.basic.READ`, `Desk.contacts.READ`, `Desk.search.READ`, `Desk.settings.READ` |

`AGORA_PUBLIC_URL` must also be set; the redirect URI is built from it, never from request headers.

**Behavior when a required variable is unset:** `GET /api/me/zoho` reports `available: false` (the UI shows "Zoho isn't set up on this server yet") and `POST /api/me/zoho/connect` returns **503**.

**Local development:** `go run ./cmd/fakezoho` serves an in-memory Zoho with two people who see different deals and tickets. Point the backend at it with `AGORA_ZOHO_CLIENT_ID=1000.FAKEZOHOCLIENT`, `AGORA_ZOHO_CLIENT_SECRET=fake-zoho-secret` and `ZOHO_DYN_ACCOUNTS_BASE` / `ZOHO_DYN_API_BASE` / `ZOHO_DYN_DESK_BASE` set to its address.

## Usage analytics

By default, the server reports to Agora's official PostHog instance. To opt out, set `ANALYTICS_DISABLED=true`.
...
- [Daemon and runtimes](/daemon-runtimes) — what the `AGORA_DAEMON_*` parameters actually do
