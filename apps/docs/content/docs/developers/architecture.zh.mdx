---
title: Architecture
description: Technical architecture of the Tandem platform.
---

## Overview

Tandem is a Go backend + monorepo frontend (pnpm workspaces + Turborepo) with shared packages.
...
- **Entry points** (`cmd/`): `server` (HTTP API), `tandem` (CLI + daemon), `migrate`
