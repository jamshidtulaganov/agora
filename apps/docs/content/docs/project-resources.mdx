...
The trade-off vs. `github_repo` is intentional: only the bound daemon can pick up tasks against the directory, and tasks on the same directory run **serially** instead of in parallel. In exchange you keep your existing checkout, your existing branch, your existing dirty state — Agora never re-clones it.
...
| Branch / dirty state | Each task gets a fresh branch from the default | Agent commits go on an isolated `agent/…` branch off your current HEAD; dirty tracked state is snapshotted first |
...
3. Pick the folder. The path is bound to **the daemon currently registered by this Desktop install** — the resource record stores both the path and that daemon's ID. Picking also records your **owner approval** for the folder (see below), so tasks can run there immediately.
...
`--daemon-id` comes from `agora daemon list`. The CLI also accepts the generic `--ref '<json>'` escape hatch if you'd rather pass the payload directly.
...
Approving a folder covers everything beneath it. A task against an unapproved path fails fast with a message that names the exact `allow-dir` command to run — approve and re-dispatch, nothing else to clean up. Resources attached from the CLI (or from another machine) always need one of the manual approval paths, since no picker ran on the daemon host.
...
- **Untracked files aren't snapshotted.** The dirty-tree backup captures tracked modifications only; an agent that overwrites or deletes an untracked file leaves nothing to recover from. Commit or stash untracked work you care about before a risky run.
