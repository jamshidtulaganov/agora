...
跟 `github_repo` 相比的取舍是有意的：只有绑定的那台守护进程会使用这个本地目录执行任务，而且 **同一个目录上的任务串行执行**，不再并行。换来的是：你的现有 checkout、当前分支、未提交的脏改动一切照旧 —— Agora 不会重新 clone。
...
| 分支 / 脏改动 | 每条任务从默认分支拉一个干净分支 | 智能体的提交落在从你当前 HEAD 切出的隔离 `agent/…` 分支上；已跟踪的脏改动会先被快照 |
...
2. **改动很细碎，需要频繁在本地 review** —— 你在反复打磨某个组件，几分钟就要切回编辑器看一眼智能体改了什么；与其每次去 `~/agora_workspaces/` 翻一个新 worktree，不如让它就在你已有的 checkout 里干活。
...
3. 选一个文件夹。这个路径会被绑定到 **当前这台桌面端注册的守护进程** —— 资源记录里同时保存路径和该守护进程的 ID。选择文件夹的同时也会记录你对它的 **机主授权**（见下文），任务可以立即在里面运行。
...
`--daemon-id` 可以通过 `agora daemon list` 拿到。CLI 也接受 `--ref '<json>'` 这种通用形式，直接传 payload。
...
授权一个文件夹即覆盖它下面的全部内容。任务指向未授权的路径时会快速失败，错误信息里直接给出要执行的 `allow-dir` 命令 —— 授权后重新派发即可，不需要清理任何东西。通过 CLI（或在另一台机器上）挂的资源总是需要手动授权，因为守护进程主机上没有跑过选择器。
...
- **未跟踪文件不进快照。** 脏改动备份只抓已跟踪的修改；智能体如果覆盖或删除了一个未跟踪文件，就没有东西可恢复。重要的未跟踪工作请在有风险的运行前先 commit 或 stash。
