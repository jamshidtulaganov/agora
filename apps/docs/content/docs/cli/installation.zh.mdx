...
description: Install the Tandem CLI and start the agent daemon.
---
...
curl -sL "https://github.com/multica-ai/multica/releases/download/${LATEST}/tandem_${OS}_${ARCH}.tar.gz" \
  -o /tmp/tandem.tar.gz
...
This configures the CLI for Tandem Cloud, opens your browser for login, discovers your workspaces, and starts the agent daemon.

For self-hosted servers, use `tandem setup self-host` instead. See [Self-Hosting](/getting-started/self-hosting) for details.
...
```bash
tandem daemon stop && tandem daemon start
```
