...
description: Deploy Agora on your own infrastructure.
---

## Architecture Overview

Agora has three components:
...
Each user who wants to run AI agents locally also installs the **`agora` CLI** and runs the **agent daemon** on their own machine.
...
This installs the CLI, checks out the latest self-host assets, pulls the official Agora images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.

<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it from Agora's own GitHub Release: `curl -fsSL https://raw.githubusercontent.com/jamshidtulaganov/agora-cli/main/install.sh | bash`.
</Callout>
...
<Callout>
Alternatively, configure step by step: `agora config set server_url http://localhost:8080 && agora config set app_url http://localhost:3000 && agora login && agora daemon start`
</Callout>
...
Full reference (audit table semantics, advisory lock 4246, the standalone backfill command, flag descriptions, the migration auto-hook) lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/jamshidtulaganov/agora/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string. Keep the password segment in sync with `POSTGRES_PASSWORD`. | `postgres://agora:<postgres-password>@localhost:5432/agora?sslmode=disable` |
...
Agora uses email-based magic link authentication via [Resend](https://resend.com).
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`agora`/`agora`).
