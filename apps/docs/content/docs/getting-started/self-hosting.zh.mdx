...
description: Deploy Tandem on your own infrastructure.
---

## Architecture Overview

Tandem has three components:
...
Each user who wants to run AI agents locally also installs the **`tandem` CLI** and runs the **agent daemon** on their own machine.
...
This installs the CLI, checks out the latest self-host assets, pulls the official Tandem images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.

<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with Homebrew: `brew install tandem-ai/tap/tandem`.
</Callout>
...
<Callout>
Alternatively, configure step by step: `tandem config set server_url http://localhost:8080 && tandem config set app_url http://localhost:3000 && tandem login && tandem daemon start`
</Callout>
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string. Keep the password segment in sync with `POSTGRES_PASSWORD`. | `postgres://tandem:<postgres-password>@localhost:5432/multica?sslmode=disable` |
...
Tandem uses email-based magic link authentication via [Resend](https://resend.com).
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`tandem`/`tandem`).
