...
1. 工作区 owner 或 admin 打开**设置 → 集成 → Zoho**，配置 **Zoho 连接器**：在 api-console.zoho.com 创建一个 *Server-based application*，注册卡片上显示的重定向 URI（`<AGORA_PUBLIC_URL>/api/integrations/zoho/callback`），再填入 Client ID、Client secret 和数据中心。Refresh token 可选——只有要把 Zoho CRM 记录同步成 issue 时才需要。
2. 之后每个人在下方的**你的 Zoho 账号**卡片里点**连接 Zoho**，用自己的身份登录 Zoho。助手或 agent 替这个人读取 Zoho 时，一律用这个人的授权，所以能看到什么由 Zoho 自己的角色、配置文件和共享规则决定。授权只包含读取权限，Agora 从不在 Zoho 里新建或修改任何东西。

服务器本身只需要：

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `AGORA_ZOHO_SECRET_KEY` | 空 | 32 字节 base64 密钥，用于加密存储连接器的 secret 和每个人的授权。不配置时无法保存连接器 |
| `AGORA_PUBLIC_URL` | 空 | 重定向 URI 由它拼出，从不取自请求头。不配置时谁都无法连接 |

**本地开发：** `go run ./cmd/fakezoho` 会启动一个内存版 Zoho，内置两个人，各自能看到的交易和工单不同。把 `ZOHO_DYN_ACCOUNTS_BASE` / `ZOHO_DYN_API_BASE` / `ZOHO_DYN_DESK_BASE` 指向它，再用 Client ID `1000.FAKEZOHOCLIENT`、Client secret `fake-zoho-secret` 配置连接器即可。
