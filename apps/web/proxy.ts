...
import { LOCALE_COOKIE } from "@agora/core/i18n";
import {
  AGORA_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-agora-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(AGORA_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("agora_logged_in");
