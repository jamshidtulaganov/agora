...
import { LOCALE_COOKIE } from "@tandem/core/i18n";
import {
  TANDEM_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-tandem-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(TANDEM_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("tandem_logged_in");
