...
const (
	tandemCodexMcpBeginMarker = "# BEGIN tandem-managed mcp_servers (do not edit; regenerated by daemon)"
	tandemCodexMcpEndMarker   = "# END tandem-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(tandemCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(tandemCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = tandemCodexMcpBeginMarker + "\n" + tandemCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(tandemCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(tandemCodexMcpEndMarker)
	sb.WriteString("\n")
