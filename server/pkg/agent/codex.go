...
const (
	agoraCodexMcpBeginMarker = "# BEGIN agora-managed mcp_servers (do not edit; regenerated by daemon)"
	agoraCodexMcpEndMarker   = "# END agora-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agoraCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agoraCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = agoraCodexMcpBeginMarker + "\n" + agoraCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(agoraCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(agoraCodexMcpEndMarker)
	sb.WriteString("\n")
