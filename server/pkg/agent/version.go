...
// the agora CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `agora issue create` failure — see PR
...
var (
	ErrCLIVersionMissing = errors.New("agora CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("agora CLI version is below required minimum")
)
