...
// the tandem CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `tandem issue create` failure — see PR
...
var (
	ErrCLIVersionMissing = errors.New("tandem CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("tandem CLI version is below required minimum")
)
