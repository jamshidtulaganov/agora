...
// emitted via the X-Agora-Next-Before / X-Agora-Next-Before-Id
// response headers.
//
...
func (h *Handler) ListComments(w http.ResponseWriter, r *http.Request) {
	...
	if result.NextBefore != "" && result.NextBeforeID != "" {
		w.Header().Set("X-Agora-Next-Before", result.NextBefore)
		w.Header().Set("X-Agora-Next-Before-Id", result.NextBeforeID)
	}
...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	if authorType == "agent" {
		...
		if verdict, labeled := h.TaskService.CaptureQAEvidence(r.Context(), issue, comment.Content, comment.ParentID); labeled {
			...
			go h.maybeCommitSpecsOnQAPass(context.Background(), issue, gateLabel, authorID)
		...
		if verdict, labeled := h.TaskService.CaptureReviewEvidence(r.Context(), issue, comment.Content, parseUUID(authorID)); labeled {
			...
			go h.onReviewVerdictLabel(context.Background(), issue, "review:"+verdict, authorID)
		}
