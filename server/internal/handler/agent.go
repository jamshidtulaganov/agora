...
// OrchestrationGitDependency identifies a completed predecessor whose commit
// an integration step must contain before it can pass the release gate.
type OrchestrationGitDependencyResponse struct {
	StepID  string                         `json:"step_id"`
	Key     string                         `json:"key"`
	Branch  string                         `json:"branch,omitempty"`
	HeadSHA string                         `json:"head_sha,omitempty"`
	Heads   []OrchestrationGitHeadResponse `json:"heads,omitempty"`
	Handoff json.RawMessage                `json:"handoff,omitempty"`
}

// OrchestrationGitHeadResponse identifies one repository commit in a
// run-level base snapshot or a completed dependency handoff.
type OrchestrationGitHeadResponse struct {
	Repo    string `json:"repo,omitempty"`
	Branch  string `json:"branch,omitempty"`
	HeadSHA string `json:"head_sha"`
}

type OrchestrationMessageEnvelopeResponse struct {
	ID        string          `json:"id"`
...
type AgentTaskResponse struct {
	...
	// PriorSessionSameOrchestrationStep is an internal daemon safety signal.
	// It allows a continuation/retry to reuse its daemon-managed local worktree,
	// while preventing a generic same-issue fallback from bypassing orchestration
	// worktree isolation. Older daemons ignore the additive field.
	PriorSessionSameOrchestrationStep bool   `json:"prior_session_same_orchestration_step,omitempty"`
	...
	ChatMessageAttachments    []ChatAttachmentMeta                   `json:"chat_message_attachments,omitempty"`    // attachments on the user message — agent calls `agora attachment download <id>` per entry
	...
	OrchestrationDependencies []OrchestrationGitDependencyResponse   `json:"orchestration_dependencies,omitempty"`
	OrchestrationMessages     []OrchestrationMessageEnvelopeResponse `json:"orchestration_messages,omitempty"`
	OrchestrationBaseRefs     []OrchestrationGitHeadResponse         `json:"orchestration_base_refs,omitempty"`
	...
	// the owner. The agent's effective Agora credentials stay owner-scoped —
	...
	// AGORA_TOKEN in the agent process environment. The server binds it to
...
// `agora attachment download <id>` rather than guessing from the
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["custom_env"]; ok {
		writeError(w, http.StatusBadRequest, "custom_env is no longer accepted on this endpoint; use PUT /api/agents/{id}/env (or `agora agent env set`)")
		return
	}
