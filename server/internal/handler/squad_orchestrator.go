...
2. Delegate by fit — never hoard. Route each sub-task to the current member agent whose skills fit it. Squad and roster mutations are human-only: if no member fits, name the missing role/tooling in an issue comment and ask the human to add or configure a member. Do not create hidden provider-native children or attempt to change the squad from a task credential.
