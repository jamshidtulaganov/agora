...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					...
					for _, row := range rows {
						...
						// so `agora repo checkout` and the meta-skill render
	...
	if task.ChatSessionID.Valid {
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			// the agent can `agora attachment download <id>` — the markdown
	...
	if task.Context != nil && !task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid {
		...
		if json.Unmarshal(task.Context, &qc) == nil && qc.Type == service.QuickCreateContextType {
			...
			// the project, and `agora repo checkout` sees the project's
	...
	// as the only authority for AGORA_WORKSPACE_ID in the agent env. An
	...
	// owner). The daemon will inject this as AGORA_TOKEN into the agent
	...
	// Auto-provision the Agora-hosted Zoho MCP proxy when the person this
	// task works for has connected Zoho. Must run after the mat_ mint above —
	// the proxy entry's only credential is that task token, so the agent's
	// Zoho calls act as that person with zero Zoho secrets leaving the
	// server. No-op without a token (ownerless runtimes), without
	// AGORA_PUBLIC_URL, or when the agent already declares a "zoho" server.
	if resp.Agent != nil && resp.AuthToken != "" {
		resp.Agent.McpConfig = h.injectZohoMcpProxy(r.Context(), task.ID, resp.Agent.McpConfig, resp.AuthToken)
	}
