...
func assistantWorkspaceGateArgs(ws, insider string) map[string]string {
	return map[string]string{
		...
		// The migration concierge. dry_run and confirm are sent with the
		// arguments they would really carry: an outsider must be refused for
		// MEMBERSHIP, not for naming a connection that does not exist in a
		// workspace they cannot see — that difference is an existence oracle.
		assistant.ToolListImportConnections: `{"workspace_id":"` + ws + `"}`,
		assistant.ToolDryRunImport:          `{"workspace_id":"` + ws + `","connection_id":"` + ws + `"}`,
		assistant.ToolUpdateImportMapping:   `{"workspace_id":"` + ws + `","status":{"Done":"done"}}`,
		assistant.ToolConfirmImport:         `{"workspace_id":"` + ws + `","job_id":"` + ws + `"}`,
		assistant.ToolImportStatus:          `{"workspace_id":"` + ws + `","job_id":"` + ws + `"}`,
