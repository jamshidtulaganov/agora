...
// translates the prompt into a `tandem issue create` invocation in the
...
// the agent's `tandem issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the tandem CLI
