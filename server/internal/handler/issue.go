...
// translates the prompt into a `agora issue create` invocation in the
...
// the agent's `agora issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the agora CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See jamshidtulaganov/agora#1660.
