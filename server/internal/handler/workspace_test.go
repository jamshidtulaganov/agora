...
//
// CompleteOnboarding (Step 3 exit) flips onboarded_at for the normal workspace
// creation flow. AcceptInvitation is the separate direct-join path and marks an
// invitee onboarded atomically with their membership.
...
`, wsID, testUserID); err != nil {
	...
	t.Run("normalizes valid repos", func(t *testing.T) {
		...
		req := newRequest("PATCH", "/api/workspaces/"+wsID, map[string]any{
			"repos": []map[string]any{
				{
					"url":         "  https://github.com/jamshidtulaganov/agora.git  ",
				...
				{
					"url": "git@github.com:jamshidtulaganov/agora-cloud.git",
				},
			},
		})
		req = withURLParam(req, "id", wsID)
		...
		if repos[0].URL != "https://github.com/jamshidtulaganov/agora.git" || repos[0].Description != "main monorepo" {
			t.Fatalf("first repo not normalized: %+v", repos[0])
		}
		if repos[1].URL != "git@github.com:jamshidtulaganov/agora-cloud.git" {
...
`, wsID, testUserID); err != nil {
	...
	targetEmail := fmt.Sprintf("revocation-%s@agora.dev", slug)
...
VALUES ($1, $2, 'Target Runtime', 'local', 'agora_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
VALUES ($1, $2, 'Other Runtime', 'local', 'agora_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
`, wsID, testUserID); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id
`, "Revocation No Runtimes Target", "revocation-no-runtimes@agora.dev").Scan(&targetUserID); err != nil {
		t.Fatalf("create target user: %v", err)
	}
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workspace WHERE id = $1`, wsID)
