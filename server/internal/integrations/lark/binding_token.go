...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	AgoraUserID  pgtype.UUID // the installer's Agora account
...
// identity is the supplied agoraUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Agora user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, agoraUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
	...
	if err != nil {
		...
		// agora_user_id differs from ours, so the WHERE clause on
		...
		// lark_user_binding_member_fk (workspace_id, agora_user_id)
...
// (installation_id, lark_open_id) when agora_user_id matches (the
...
// open_id) pair and points at a different Agora user. Account
