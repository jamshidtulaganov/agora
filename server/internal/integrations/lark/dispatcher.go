...
type DispatchResult struct {
	...
	// even when the Agora deep link is not reachable.
	IssueTitle string
}
...
func (d *Dispatcher) processClaimed(ctx context.Context, msg InboundMessage, inst db.LarkInstallation, claimToken pgtype.UUID) (DispatchResult, dedupFinalize, error) {
	...
	//    so we use them.
	sessionCreator := binding.AgoraUserID
	...
	if appendRes.IssueCommand != nil {
		issueRes, err := d.createIssueFromCommand(ctx, inst, binding.AgoraUserID, sessionID, *appendRes.IssueCommand)
		if err != nil {
	...
	// binding.AgoraUserID is THIS message's sender — the task initiator. It is
	...
	d.scheduleRun(inst, msg, sessionID, binding.AgoraUserID)
