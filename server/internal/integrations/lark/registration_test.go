...
func TestRegistrationClient_Begin_HappyPath(t *testing.T) {
	...
	res, err := c.Begin(context.Background(), "Ada - Agora", "")
	if err != nil {
	...
	if !strings.HasPrefix(q.Get("source"), "go-sdk/agora") {
		t.Errorf("qr source=%q want go-sdk/agora", q.Get("source"))
	}
	// The name preset pre-fills the Lark PersonalAgent creation form so
	// the bot defaults to "<agent> - Agora" rather than the
	// auto-generated "{用户姓名}的智能助手".
	if q.Get("name") != "Ada - Agora" {
		t.Errorf("qr name=%q want %q", q.Get("name"), "Ada - Agora")
	}
}
