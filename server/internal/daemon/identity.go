...
// same machine shares one UUID stored at `~/.agora/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.agora/profiles/*/daemon.id` and returns every
...
// explicitly pins AGORA_DAEMON_ID to the hostname itself, there's nothing
