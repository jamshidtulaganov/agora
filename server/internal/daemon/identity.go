...
// same machine shares one UUID stored at `~/.tandem/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.tandem/profiles/*/daemon.id` and returns every
...
// explicitly pins TANDEM_DAEMON_ID to the hostname itself, there's nothing
