...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "tandem login") {
		t.Fatalf("401 warning must tell the user to run 'tandem login', got: %s", out)
	}
}
...
// and the operator would never see a "run tandem login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "tandem login") {
		t.Fatalf("expected the actionable 'run tandem login' hint in the WARN, got: %s", out)
	}
}
