...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "agora login") {
		t.Fatalf("401 warning must tell the user to run 'agora login', got: %s", out)
	}
}
...
// and the operator would never see a "run agora login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "agora login") {
		t.Fatalf("expected the actionable 'run agora login' hint in the WARN, got: %s", out)
	}
}
