...
type Task struct {
	...
	// The daemon injects it into the spawned agent as AGORA_TOKEN so the
...
// merge. The daemon verifies the commit ancestry independently of agent output.
// Branch/HeadSHA describe the dependency's primary repo; Heads carries one
// entry per repo for multi-repo local_directory projects. Old servers omit
// Heads and old daemons ignore it, so both sides fall back to the single pair.
type OrchestrationGitDependency struct {
...
type OrchestrationGitHead struct {
	Repo    string `json:"repo,omitempty"`
	Branch  string `json:"branch,omitempty"`
	HeadSHA string `json:"head_sha,omitempty"`
}

// RepoGitState is the per-repository git evidence reported when an
// orchestration task completes. One entry per provisioned worktree repo.
...
// run `agora attachment download <id>` instead of guessing from a
