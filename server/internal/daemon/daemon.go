...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'agora login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'agora login --profile %s'", d.cfg.Profile)
		}
...
// `agora repo checkout` accepts project-only URLs without an extra round
...
// other API call is what surfaces a user-actionable "run agora login"
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Agora Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/agora)
...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/agora symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "agora")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "agora")
		} else {
...
// release. Used by pollLoop to expose AGORA_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, taskErr error) {
	...
	// /agora_workspaces/{ws}/{short-id}/workdir hit FileNotFoundError in
	...
	// AGORA_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// AGORA_TOKEN is the credential the agent process will use to call the
	// Agora API. Prefer the task-scoped token the server minted at claim
	...
	agentEnv := map[string]string{
		"AGORA_TOKEN":        agentToken,
		"AGORA_SERVER_URL":   d.cfg.ServerBaseURL,
		"AGORA_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"AGORA_WORKSPACE_ID": task.WorkspaceID,
		"AGORA_AGENT_NAME":   agentName,
	...
	if task.AutopilotID != "" {
		agentEnv["AGORA_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the agora CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["AGORA_QUICK_CREATE_TASK_ID"] = task.ID
		if len(task.QuickCreateAttachmentIDs) > 0 {
			if raw, err := json.Marshal(task.QuickCreateAttachmentIDs); err == nil {
				agentEnv["AGORA_QUICK_CREATE_ATTACHMENT_IDS"] = string(raw)
			} else {
				taskLog.Warn("quick-create attachment ids: marshal failed; skipping env injection", "error", err)
			}
		}
	}
	// Ensure the agora CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// agora binary so that `agora` commands in the agent always resolve.
	...
	// `agora issue status` / `agora issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "AGORA_") {
		return true
	}
	switch upper {
