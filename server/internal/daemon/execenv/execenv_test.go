...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/jamshidtulaganov/agora","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".agora", "project", "resources.json")
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".agora", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agora autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `agora issue get") {
...
// collision-free duplicates (issue-review, issue-review-agora,
// issue-review-agora-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -agora duplicates", names)
	}
...
// next refresh dodges to issue-review-agora. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -agora duplicate", names)
	}
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agora issue create --title",
		"agora issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"agora repo checkout <url>",
		"agora issue status <id> <status>",
		"agora issue comment add <issue-id>",
	...
	for _, banned := range []string{
		"agora issue list [--status",
		"agora issue label list",
		"agora issue subscriber list",
		"agora label list",
		"agora workspace member list",
		"agora agent list",
		"agora squad list",
		"agora issue runs",
		"agora issue run-messages",
		"agora attachment download",
		"agora autopilot list",
		"agora autopilot create",
		"agora autopilot update",
		"agora autopilot trigger",
		"agora autopilot delete",
		"agora project get",
		"agora project resource list",
		"agora issue assign",
		"agora issue label add",
		"agora issue label remove",
		"agora issue subscriber add",
		"agora issue subscriber remove",
		"agora issue comment delete",
		"agora label create",
	} {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Agora Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "agora issue get") {
		t.Error("should reference agora CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `agora issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"agora issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `agora issue comment add`",
...
// non-ASCII as `?` before reaching `agora.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		agoraManagedBeginMarker,
		agoraMultiAgentBeginMarker,
		agoraMemoryFeatureBeginMarker,
		agoraMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, agoraManagedBeginMarker) || !strings.Contains(s, agoraManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertAgoraManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), agoraManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.agora.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, agoraManagedBeginMarker)
	endIdx := strings.Index(s, agoraManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.agora]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.agora]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.agora content")
	}
	...
	if n := strings.Count(string(data2), agoraManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.agora], got:\n%s", s)
	}
	if strings.Count(s, agoraManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), agoraManagedBeginMarker) {
		t.Error("reused config.toml missing agora-managed block")
	}
}
...
// inside a Agora task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigCommentTriggerColdStartRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agora issue comment list " + issueID + " --thread " + triggerID + " --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "agora issue comment list "+issueID+" --thread "+triggerID+" --output json") {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"agora issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"agora issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"agora issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `agora issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"agora issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"agora issue metadata set",
				"agora issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_heredoc", func(t *testing.T) {
		...
		if !strings.Contains(s, "agora issue metadata list issue-md-codex --output json") {
