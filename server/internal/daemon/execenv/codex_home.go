...
type CodexHomeOptions struct {
	...
	// WritableRoots are additional owner-approved project folders exposed to
	// Codex when the task runs in workspace-write mode. The task working
	// directory itself is implicitly writable and must not be repeated here.
...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Agora writes the
	...
	// so cross-task and cross-workspace context leaks (agora#3130) cannot
