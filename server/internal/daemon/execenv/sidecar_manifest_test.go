...
// nothing related to Tandem before a task ran must contain nothing
// related to Tandem after the task finishes — no .agent_context/,
// no .claude/skills/, no .tandem/, no stub directories. The test
...
// must still exist and the Tandem-written sibling must be gone.
...
// round-trip to the .tandem/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .tandem/sidecar.txt and created
	// .tandem/ + .tandem/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .tandem/.
	managedDir := filepath.Join(workDir, ".tandem")
	...
	// .tandem still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-tandem/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Tandem would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Tandem skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Tandem
// sibling (which lives at `<slug>-tandem`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Tandem's version",
						Content:     "---\nname: issue-review\n---\n\nTandem skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Tandem-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.tandem/project/
// resources.json` — the other Tandem-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".tandem", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".tandem", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-tandem`, then `-tandem-2`,
// `-tandem-3`, … The PR-review concern is "Tandem skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-tandem`.
	...
	if slug != "issue-review-tandem" {
		t.Errorf("second allocation should bump to `-tandem`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-tandem") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-tandem"), 0o755); err != nil {
	...
	if slug != "issue-review-tandem-2" {
		t.Errorf("third allocation should be `-tandem-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-tandem-2") {
...
// `Issue Review` skill, the Tandem sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Tandem sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Tandem skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Tandem's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	tandemDir := filepath.Join(workDir, ".claude", "skills", "issue-review-tandem")
	if _, err := os.Stat(filepath.Join(tandemDir, "SKILL.md")); err != nil {
		t.Errorf("Tandem sibling skill should exist at %s: %v", tandemDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Tandem sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(tandemDir); !os.IsNotExist(err) {
		t.Errorf("Tandem sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
