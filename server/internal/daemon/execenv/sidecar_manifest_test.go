...
// nothing related to Agora before a task ran must contain nothing
// related to Agora after the task finishes — no .agent_context/,
// no .claude/skills/, no .agora/, no stub directories. The test
...
// must still exist and the Agora-written sibling must be gone.
...
// round-trip to the .agora/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .agora/sidecar.txt and created
	// .agora/ + .agora/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .agora/.
	managedDir := filepath.Join(workDir, ".agora")
	...
	// .agora still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-agora/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Agora would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Agora skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Agora
// sibling (which lives at `<slug>-agora`) must be fully removed by
// CleanupSidecars.
...
// — a Agora-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.agora/project/
// resources.json` — the other Agora-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".agora", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".agora", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-agora`, then `-agora-2`,
// `-agora-3`, … The PR-review concern is "Agora skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-agora`.
	...
	if slug != "issue-review-agora" {
		t.Errorf("second allocation should bump to `-agora`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-agora") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-agora"), 0o755); err != nil {
	...
	if slug != "issue-review-agora-2" {
		t.Errorf("third allocation should be `-agora-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-agora-2") {
...
// `Issue Review` skill, the Agora sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Agora sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Agora skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Agora's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	agoraDir := filepath.Join(workDir, ".claude", "skills", "issue-review-agora")
	if _, err := os.Stat(filepath.Join(agoraDir, "SKILL.md")); err != nil {
		t.Errorf("Agora sibling skill should exist at %s: %v", agoraDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Agora sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(agoraDir); !os.IsNotExist(err) {
		t.Errorf("Agora sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
