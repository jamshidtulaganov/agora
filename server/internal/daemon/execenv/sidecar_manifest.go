...
const sidecarManifestFile = ".agora_sidecar_manifest.json"
...
//     so the agent still discovers the Agora skill, just under a
//     different directory name.
//   - For Agora-only namespaces (.agent_context/issue_context.md,
//     .agora/project/resources.json) the caller swallows the error
...
// down a Agora skill without colliding with a user-installed skill
...
// collision we append `-agora`, then `-agora-2`, `-agora-3`,
...
func allocateCollisionFreeSkillDir(skillsParent, baseSlug string) (slug, dir string, err error) {
	...
	for i := 0; i < maxAttempts; i++ {
		...
		case i == 0:
			candidate = baseSlug
		case i == 1:
			candidate = baseSlug + "-agora"
		default:
			candidate = fmt.Sprintf("%s-agora-%d", baseSlug, i)
		}
...
// (.agent_context/, .agora/, .claude/skills/, .github/skills/,
...
// issue-review-agora. A managed skill directory is platform-owned — the
