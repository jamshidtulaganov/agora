...
// `no such host` errors when calling out (for example, `agora issue get`
// hitting the Agora API). See upstream issue openai/codex#10390.
//
...
// actually reach the Agora API. On Linux (and on macOS once the upstream
...
//     danger-full-access so the Agora CLI can reach the API.
...
// agoraManagedBeginMarker / agoraManagedEndMarker delimit the block the
...
const (
	agoraManagedBeginMarker = "# BEGIN agora-managed (do not edit; regenerated by daemon)"
	agoraManagedEndMarker   = "# END agora-managed"
)

// renderAgoraManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.agora]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertAgoraManagedBlock), avoids both traps.
func renderAgoraManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(agoraManagedBeginMarker)
	b.WriteString("\n")
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agoraManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agoraManagedEndMarker) + `\n*`)

// upsertAgoraManagedBlock returns the config content with the agora-managed
...
// `[permissions.agora]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderAgoraManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertAgoraManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderAgoraManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the agora-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertAgoraManagedBlock(existing, policy)
