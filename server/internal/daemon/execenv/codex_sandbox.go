...
// `no such host` errors when calling out (for example, `tandem issue get`
// hitting the Tandem API). See upstream issue openai/codex#10390.
//
...
// actually reach the Tandem API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Tandem CLI can reach the API.
...
// tandemManagedBeginMarker / tandemManagedEndMarker delimit the block the
...
const (
	tandemManagedBeginMarker = "# BEGIN tandem-managed (do not edit; regenerated by daemon)"
	tandemManagedEndMarker   = "# END tandem-managed"
)

// renderTandemManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.tandem]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertTandemManagedBlock), avoids both traps.
func renderTandemManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(tandemManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(tandemManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(tandemManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(tandemManagedEndMarker) + `\n*`)

// upsertTandemManagedBlock returns the config content with the tandem-managed
...
// `[permissions.tandem]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderTandemManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertTandemManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderTandemManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the tandem-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertTandemManagedBlock(existing, policy)
