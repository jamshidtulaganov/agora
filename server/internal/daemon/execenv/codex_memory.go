...
// — users cannot audit or edit the contents from any Agora UI.
//
// This conflicts with the Agora daemon's context model. Agora
...
//     unrelated host-project context into Agora tasks. The reproduction
//     in github.com/jamshidtulaganov/agora#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Agora issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Agora task
// (and accept the leak risk) can keep the feature enabled by setting
// `AGORA_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Agora-owned, user-visible,
...
// AgoraCodexMemoryEnv is the env var users can set to keep Codex
...
const AgoraCodexMemoryEnv = "AGORA_CODEX_MEMORY"

const (
	agoraMemoryFeatureBeginMarker = "# BEGIN agora-managed memory-feature (do not edit; regenerated by daemon)"
	agoraMemoryFeatureEndMarker   = "# END agora-managed memory-feature"
	agoraMemoryConfigBeginMarker  = "# BEGIN agora-managed memory-config (do not edit; regenerated by daemon)"
	agoraMemoryConfigEndMarker    = "# END agora-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agoraMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agoraMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agoraMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agoraMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(AgoraCodexMemoryEnv))
...
// renderAgoraMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderAgoraMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(agoraMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(agoraMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderAgoraMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderAgoraMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(agoraMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(agoraMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderAgoraMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderAgoraMemoryConfigBlock(true))
}
...
//
// When AGORA_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per AGORA_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderAgoraMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderAgoraMemoryConfigBlock(false))
	}
