...
// — users cannot audit or edit the contents from any Tandem UI.
//
// This conflicts with the Tandem daemon's context model. Tandem
...
//
// Users who explicitly want Codex native memory inside a Tandem task
// (and accept the leak risk) can keep the feature enabled by setting
// `TANDEM_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Tandem-owned, user-visible,
...
// TandemCodexMemoryEnv is the env var users can set to keep Codex
...
const TandemCodexMemoryEnv = "TANDEM_CODEX_MEMORY"

const (
	tandemMemoryFeatureBeginMarker = "# BEGIN tandem-managed memory-feature (do not edit; regenerated by daemon)"
	tandemMemoryFeatureEndMarker   = "# END tandem-managed memory-feature"
	tandemMemoryConfigBeginMarker  = "# BEGIN tandem-managed memory-config (do not edit; regenerated by daemon)"
	tandemMemoryConfigEndMarker    = "# END tandem-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(tandemMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(tandemMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(tandemMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(tandemMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(TandemCodexMemoryEnv))
...
// renderTandemMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderTandemMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(tandemMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(tandemMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderTandemMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderTandemMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(tandemMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(tandemMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderTandemMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderTandemMemoryConfigBlock(true))
}
...
//
// When TANDEM_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per TANDEM_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderTandemMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderTandemMemoryConfigBlock(false))
	}
