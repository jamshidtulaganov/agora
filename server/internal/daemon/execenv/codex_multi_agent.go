...
// subagents. The Tandem daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Tandem runtime
...
//
// Users who explicitly want Codex native subagents inside a Tandem task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `TANDEM_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// TandemCodexMultiAgentEnv is the env var users can set to keep Codex
...
const TandemCodexMultiAgentEnv = "TANDEM_CODEX_MULTI_AGENT"

// tandemMultiAgentBeginMarker / tandemMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	tandemMultiAgentBeginMarker = "# BEGIN tandem-managed multi-agent (do not edit; regenerated by daemon)"
	tandemMultiAgentEndMarker   = "# END tandem-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(tandemMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(tandemMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(TandemCodexMultiAgentEnv))
...
// renderTandemMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderTandemMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(tandemMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(tandemMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderTandemMultiAgentBlock(true)
...
//
// When TANDEM_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per TANDEM_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderTandemMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
