...
// subagents. The Agora daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Agora runtime
...
// AgoraCodexMultiAgentEnv is the env var users can set to keep Codex
...
const AgoraCodexMultiAgentEnv = "AGORA_CODEX_MULTI_AGENT"

// agoraMultiAgentBeginMarker / agoraMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	agoraMultiAgentBeginMarker = "# BEGIN agora-managed multi-agent (do not edit; regenerated by daemon)"
	agoraMultiAgentEndMarker   = "# END agora-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agoraMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agoraMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(AgoraCodexMultiAgentEnv))
...
// renderAgoraMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderAgoraMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(agoraMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(agoraMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderAgoraMultiAgentBlock(true)
...
//
// When AGORA_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderAgoraMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
