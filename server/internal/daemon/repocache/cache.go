...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/agora_workspaces/.repos)
	logger *slog.Logger
...
type WorktreeParams struct {
	...
	BranchName          string // optional server-selected issue branch
...
func (c *Cache) CreateWorktree(params WorktreeParams) (*WorktreeResult, error) {
	...
	// Prefer the server-selected feature/<issue> or fix/<issue> branch. Older
	// servers omit it, so mixed-version deployments retain the legacy name.
...
// agoraHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const agoraHookMarker = "# agora:prepare-commit-msg:co-authored-by"
...
// treats a hook as Agora-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before agoraHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	agoraHookMarker,
	"# Installed by the Agora daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Agora Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# agora:prepare-commit-msg:co-authored-by
# Agora: add Co-authored-by trailer for the Agora Agent.
# Installed by the Agora daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: agora-agent <github@agora.dev>"
...
// a Co-authored-by trailer for the Agora Agent. The hook is installed in the
...
// installed by the Agora daemon (current or any previously released
