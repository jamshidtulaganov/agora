---
name: tandem-projects-and-resources
description: "Use when creating, inspecting, updating, or debugging Tandem projects and project resources. Covers durable project context, github_repo and local_directory resources, how resources affect future agent task context, when to bind repos, and when not to mutate resources."
user-invocable: false
allowed-tools: Bash(tandem *)
---

# Tandem Projects and Resources
...
tandem project get <project-id> --output json
...
A project groups work and carries durable resources. A resource is not just display metadata; it is context later injected into task briefs and `.tandem/project/resources.json`.
...
tandem project list --output json
tandem project get <project-id> --output json
tandem project create --title "<title>" --repo <github-url> --output json
tandem project update <project-id> --title "<title>" --output json
tandem project status <project-id> in_progress --output json
tandem project resource list <project-id> --output json
tandem project resource add <project-id> --type github_repo --url <github-url> --output json
tandem project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --output json
tandem project resource update <project-id> <resource-id> --url <new-github-url> --output json
tandem project resource remove <project-id> <resource-id> --output json
```
...
Project resources are durable and affect future tasks. `tandem repo checkout`
is task-local checkout state.

## Debugging wrong context

1. `tandem project get <project-id> --output json`.
2. `tandem project resource list <project-id> --output json`.
