...
server/internal/service/review_evidence.go  # ParseReviewResultBlock, CaptureReviewEvidence (label-first),
                                             # LatestReviewResultForIssue (newest-first comment scan)
...
server/internal/handler/merge_readiness.go  # requiredGatesWithReview — "review" required for full tier + PR
```
...
- Reviewer ≠ author invariant: `resolveReviewerAgent` never returns the
  issue's assignee agent; the manual slice action
  (`POST /slice-actions {kind:"run_review"}`) also skips the assignee
  default for this kind (slice_action.go resolveSliceActionAgent a.55).
...
- `maybeMergeOnQAPass` now accepts both `qa:pass` and `review:pass` trigger
  labels; when `reviewGateApplies` (full tier + PR) it proceeds only with
  BOTH verdicts present, and its human-facing READY note is deduped by
  `<!-- ready-for-human-merge -->`.
- `POST /review-decision` (human-only): `approve` requires qa:pass present
  and review:fail absent (`merge:override` bypasses), attaches
  `merge:approved` (#2563eb), posts a system comment, and dispatches a
  member-authored `gh pr merge` order to the dev squad leader;
