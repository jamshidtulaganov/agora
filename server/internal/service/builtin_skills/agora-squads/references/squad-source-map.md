...
- `server/internal/handler/slice_action.go` `maybeRunQAOnInReview(...)` resolves the non-sprint smoke target as dev_apps URL (`devLocalAppURL`, gated on `labs.qa_dev_runtimes`) → `userDevServerURL(...)` (the assignee developer's standing dev server for the project, `user_dev_server` table via `server/internal/handler/qa_target.go` / `user_dev_server.go`; declaring the URL is the opt-in, no labs gate) → `localDirectoryQATarget(...)` (a project `local_directory` on an ONLINE daemon; attaching it is the opt-in, no labs gate). When the local_directory tier wins it appends `qaLocalDirectoryClause(localPath)` (start-via-`/editor/preview` + smoke `127.0.0.1` + tree-safety worktree baseline) instead of a `SMOKE TARGET` URL; when the dev-server tier wins the `SMOKE TARGET` clause tells QA to first verify the box actually serves this task's change.
