...
```bash
agora agent get <agent-id> --output json      # full persisted agent record
agora agent skills list <agent-id> --output json   # current skill bindings
agora agent env get <agent-id> --output json  # plaintext env (owner/admin only, agents denied)
```
...
`POST /api/agents` (`agora agent create`). At task claim time the daemon
...
```bash
agora agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
agora agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`agora agent env get`). It is gated to workspace **owner/admin**
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`agora agent env set`), which is owner/admin-only
...
```bash
agora agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
agora agent update <agent-id> --mcp-config-stdin --output json
agora agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
`zoho_desk_list_tickets`, `zoho_desk_get_ticket`,
`zoho_desk_ticket_conversation` — and act ONLY as that person — whoever
...
issue's person) — using their own Zoho grant, so Zoho applies their role and
sharing rules. There is no fallback to the runtime owner, to anyone else's
account, or to the workspace's org-level connection: if no person can be
identified, or they haven't connected Zoho, there is no `zoho` entry and any
direct call returns an error saying so. An agent-level `zoho` server entry
suppresses the auto-provisioned one.
...
- `agora agent create` — inserts a new agent row.
- `agora agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `agora agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
