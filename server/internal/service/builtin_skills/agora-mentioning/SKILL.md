...
This skill states WHAT a mention link does in the Agora backend, traced to
...
- a person → `agora workspace member list --output json` → use `user_id`
- an agent → `agora agent list --output json` → use `id`
- a squad  → `agora squad list --output json` → use `id`
...
Correct:
  1. `agora workspace member list --output json`  → Alice's `user_id` = 7f3a…
