...
Evidence layer for `agora-skill-importing`. Every behavioral claim in `SKILL.md`
...
grep -n "func runSkillImport"           server/cmd/agora/cmd_skill.go
...
## CLI: `agora skill import --url`

| Behavior | File:line |
|---|---|
| `skill import` command def | `server/cmd/agora/cmd_skill.go:60-64` |
| `--url` flag | `server/cmd/agora/cmd_skill.go:142` |
| `--on-conflict` flag (default `fail`) | `server/cmd/agora/cmd_skill.go:143` |
| `--output` flag (default `json`) | `server/cmd/agora/cmd_skill.go:144` |
| `runSkillImport` | `server/cmd/agora/cmd_skill.go:412` |
| Requires `--url` | `server/cmd/agora/cmd_skill.go:418-421` |
| Reads and validates `--on-conflict` | `server/cmd/agora/cmd_skill.go:422-425` |
| Sends `on_conflict` in the request body | `server/cmd/agora/cmd_skill.go:428-431` |
| `POST /api/skills/import` | `server/cmd/agora/cmd_skill.go:436` |
| Structured HTTP error body handling | `server/cmd/agora/cmd_skill.go:437-440`, `handleSkillImportError` at `:454` |
| Prints structured result (`json` or table) | `server/cmd/agora/cmd_skill.go:443`, helper at `:497` |
...
| CLI normalizes legacy `{existing_skill}` body into `status:"conflict"` | `server/cmd/agora/cmd_skill.go:454-482`, helper at `:484` |
...
| CLI `agent skills add` def ("without replacing existing assignments") | `server/cmd/agora/cmd_agent.go:125-130` |
| `runAgentSkillsAdd` → `POST .../skills/add` | `server/cmd/agora/cmd_agent.go:797`; POST `:818` |
| CLI `agent skills set` def ("replaces all current assignments") | `server/cmd/agora/cmd_agent.go:118-123` |
| `runAgentSkillsSet` → `PUT .../skills` | `server/cmd/agora/cmd_agent.go:772`; PUT `:790` |
| CLI `agent skills list` | `server/cmd/agora/cmd_agent.go:740`; GET `:750` |
