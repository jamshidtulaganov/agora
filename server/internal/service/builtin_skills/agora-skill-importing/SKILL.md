---
name: agora-skill-importing
description: "Use when a user provides a skill URL, slug, or clear intent to import/install a specific skill into the current Agora workspace. Teaches the workspace import API/CLI path (POST /api/skills/import), the supported URL source families, --on-conflict fail|overwrite|rename|skip behavior and structured import results, additive agent binding vs replace-all, and the reserved SKILL.md supporting-file rule. Do not use it to decide which skill the user needs, and never treat an external local installer like npx skills add as the final Agora install."
user-invocable: false
allowed-tools: Bash(agora *)
---

# Importing skills into Agora

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Agora workspace.
...
A skill is installed for Agora only when it exists in the current workspace's
...
environment, not the Agora workspace DB, so Agora cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `agora skill import --url <url> --output json`:

```bash
agora skill import --url clawhub.ai/owner/skill --output json
agora skill import --url skills.sh/owner/repo/skill --output json
agora skill import --url github.com/owner/repo --output json
agora skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
agora skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
After the final `agora agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`agora agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`agora agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `agora skill import --url <url>` is equivalent to
...
agora skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
agora skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
agora skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
agora skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
agora skill get <skill-id> --output json
```
...
Incorrect (bypasses Agora):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Agora cannot manage it as a workspace skill.
...
```bash
agora skill import --url https://skills.sh/owner/repo/skill --output json
```
