...
- `server/internal/handler/bitrix_sync.go` stamps responsible and creator identity metadata used by Bitrix-scoped notification rules; `server/internal/integrations/bitrix/client.go` selects both `RESPONSIBLE_ID` and `CREATED_BY` on every task fetch path.
