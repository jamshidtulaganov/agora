...
## `tandem issue pull-requests` — read PR links from Tandem

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/tandem/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/tandem/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/tandem/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `tandem issue pull-requests` later reads back.
...
| Behavior | File:line |
|---|---|
| `tandem issue metadata set <issue-id> --key --value [--type]` | `server/cmd/tandem/cmd_issue_metadata.go:80,109-111` |
| `tandem issue metadata delete <issue-id> --key` | `server/cmd/tandem/cmd_issue_metadata.go:93,113` |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/tandem/cmd_issue.go
