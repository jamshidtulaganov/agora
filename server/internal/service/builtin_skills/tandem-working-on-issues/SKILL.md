---
name: tandem-working-on-issues
description: "Use when working on a Tandem issue after the runtime has provided the trigger context — to apply the product contracts the runtime brief does not encode: how PR linking differs from close intent, how to read a linked PR's real state via the pull-requests CLI, which metadata keys are high-signal, what status changes trigger on the server, and how sub-issue create status (todo vs backlog) controls whether assigned agents start immediately."
user-invocable: false
allowed-tools: Bash(tandem *), Bash(git *), Bash(gh *)
---

# Working on Tandem issues
...
For building mention links, load `tandem-mentioning` instead — not this skill.
...
This is the link that `tandem issue pull-requests` reads back.
...
is to open or update a PR before posting the final Tandem issue comment, unless
...
When a step depends on PR state, query Tandem's link table — do not infer it
...
```bash
tandem issue metadata set <issue-id> --key pr_url --value <url>
tandem issue metadata delete <issue-id> --key <stale-key>
```
...
```bash
tandem issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
tandem issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
tandem issue status <child-id> todo   # promote when the previous step is truly done
```
...
tandem issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
tandem issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — parked, promote in turn
tandem issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status backlog
tandem issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status backlog
```
