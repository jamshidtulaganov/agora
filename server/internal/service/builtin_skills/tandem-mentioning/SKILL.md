...
This skill states WHAT a mention link does in the Tandem backend, traced to
...
- a person → `tandem workspace member list --output json` → use `user_id`
- an agent → `tandem agent list --output json` → use `id`
- a squad  → `tandem squad list --output json` → use `id`
...
Correct:
  1. `tandem workspace member list --output json`  → Alice's `user_id` = 7f3a…
