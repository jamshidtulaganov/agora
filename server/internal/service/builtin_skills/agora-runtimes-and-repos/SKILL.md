...
- `local_directory` resources point at a path owned by a daemon and carry local-machine assumptions. They are **human-only to mutate**: create/update/delete with an agent task token or cloud PAT returns 403 (do not retry — ask the user to attach the directory themselves, from the web project panel's "Add local folder", the desktop picker, or the CLI). The ref's `daemon_id` must also resolve to a daemon runtime the human caller may use (runtime owner, workspace owner/admin, or `visibility=public`); an unregistered daemon id is a 400.
...
- One project may attach multiple folders to one daemon. The first position-ordered row is primary; later rows are additional read/write roots. The daemon validates and approves every path, locks all in-place roots in canonical order, and gives Codex the additional paths through its managed writable-roots sandbox configuration. A duplicate daemon + path or two symlink aliases of one real folder fail closed.
- The web folder picker (`GET /api/runtimes/by-daemon/{daemonId}/browse` → `GET <daemon_url>/editor/fs/list`) only **lists** directories so a human can pick a path without typing it; the daemon serves just its browsable roots (the machine's home plus already-approved folders) and the listing grants nothing. Browsing is not approval — a folder attached that way still needs `agora daemon allow-dir` before tasks run in it.
- When an in-place local_directory is a git repo, the daemon isolates the run independently for every attached root: it snapshots any dirty tracked state to `refs/agora/backup/<short-task>` (`git stash create`, zero-touch), then runs the agent on a throwaway `agent/<name>/<short-task>` branch cut from the user's HEAD. If the agent makes **no commits** (a QA gate, an inspection) the branch is torn down and the user's original branch is restored. If the agent **did** commit, that working tree stays on the agent branch and the combined result comment names the branches + checkout commands. The user's own branch refs are never moved; untracked files are not in the snapshot. If any row requests `worktree` or read-only mode, all attached Git roots are provisioned as sibling issue worktrees instead.
