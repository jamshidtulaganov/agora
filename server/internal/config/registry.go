...
var Registry = []Def{
	...
	{Key: "AGORA_TELEGRAM_REVIEW_NOTIFY_ENABLED", Kind: KindBool, Category: "Review", Label: "Telegram notice on review verdict", Description: "Post the code-review outcome (verdict, summary, blocker count, owner, next step) to the room this issue's work speaks in. The destination is RESOLVED, not configured: the speaker agent's own bot in its own bound group (Settings → Integrations → Telegram → Connect a bot → Add group), falling back to the platform bot with AGORA_TELEGRAM_REPORT_CHAT_ID when one is set. No chat id is required. Per-USER DMs need no flag either: a verdict already writes an inbox item and the bot DMs each recipient. Default off.", ProjectScoped: true},
	...
	{Key: "AGORA_TELEGRAM_REPORT_CHAT_ID", Kind: KindString, Category: "Automation", Label: "Telegram report chat", Description: "OPTIONAL override. Telegram chat the PLATFORM bot posts team notices to when no agent bot + bound group resolves: new-issue notifications, completed autopilot reports, review verdicts and automation notices. Group ids are negative (e.g. -1001234567890) \u2014 add the bot to the group first. Empty disables posting. Project-scoped, so different projects can report to different chats.", ProjectScoped: true},
	...
	{Key: "AGORA_ASSISTANT_SCHEDULES_ENABLED", Kind: KindBool, Category: "Assistant", Label: "Scheduled report refreshes", Description: "Kill switch for scheduled report refreshes: with it off the ticker starts nothing, existing cadences stay configured and simply do not fire. Each due schedule starts one ordinary assistant run under the report owner's identity, so this is the one assistant surface that spends money with nobody watching \u2014 turning it off here stops all of it instance-wide with no redeploy.", Default: "true"},
