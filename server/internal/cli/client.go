...
// Set by the tandem binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Tandem server API.
...
// reading a capped slice of the body. Every Tandem API helper funnels its
...
// It can be overridden with the TANDEM_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring TANDEM_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("TANDEM_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors TANDEM_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects TANDEM_HTTP_TIMEOUT.
