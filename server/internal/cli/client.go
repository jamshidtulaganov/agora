...
// Set by the agora binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Agora server API.
...
// reading a capped slice of the body. Every Agora API helper funnels its
...
// It can be overridden with the AGORA_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring AGORA_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("AGORA_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors AGORA_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects AGORA_HTTP_TIMEOUT.
