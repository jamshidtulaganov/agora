...
func buildSystemPrompt(uc UserContext, summary string) string {
	...
	b.WriteString("- COUNT from list_issues for anything wider than the user's own issues. list_my_issues returns ")
	b.WriteString("only what is assigned to this person, so using it for \"how many bugs are open\" under-counts ")
	b.WriteString("the workspace. Same rule for anything you put in a chart.\n")
	b.WriteString("- For the user's OWN issues (\"how many tasks do I have\", \"how many of mine are done\"), ")
	b.WriteString("list_my_issues is the right tool: its total and by_status are exact counts over ALL of them, ")
	b.WriteString("even when the issues list is truncated. Quote those numbers; never count the rows.\n")
