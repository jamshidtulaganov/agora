...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("TANDEM_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("TANDEM_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("TANDEM_WORKSPACE_ID")
	agentName := os.Getenv("TANDEM_AGENT_NAME")
	taskID := os.Getenv("TANDEM_TASK_ID")
