...
func TestPrintDiskUsageEmptyHintSuggestsProfilesWithTasks(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("TANDEM_WORKSPACES_ROOT", "")
	...
	printDiskUsageEmptyHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "tandem_workspaces"),
	}, "", "")
	...
	if !strings.Contains(got, "tandem --profile two-tasks daemon disk-usage") {
		t.Fatalf("hint output = %q, want two-tasks profile command", got)
	}
	if !strings.Contains(got, "tandem --profile one-task daemon disk-usage") {
		t.Fatalf("hint output = %q, want one-task profile command", got)
	}
	if !strings.Contains(got, "tandem --profile 'space profile' daemon disk-usage") {
...
func TestPrintDiskUsageEmptyHintSuggestsDefaultFromNamedProfile(t *testing.T) {
	...
	var out bytes.Buffer
	printDiskUsageEmptyHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "tandem_workspaces_named"),
	}, "named", "")

	got := out.String()
	if !strings.Contains(got, "tandem daemon disk-usage") {
...
func mkdirProfile(t *testing.T, home, profile string) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(home, ".tandem", "profiles", profile), 0o755); err != nil {
		t.Fatal(err)
	}
}
...
func writeDiskUsageTaskFile(t *testing.T, home, profile, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "tandem_workspaces_"+profile, workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}

func writeDefaultDiskUsageTaskFile(t *testing.T, home, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "tandem_workspaces", workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}
