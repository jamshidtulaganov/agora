...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"overridable via TANDEM_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: TANDEM_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: TANDEM_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: TANDEM_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: TANDEM_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: TANDEM_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: TANDEM_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: TANDEM_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: TANDEM_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: TANDEM_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: TANDEM_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: TANDEM_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: TANDEM_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: TANDEM_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: TANDEM_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: TANDEM_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: TANDEM_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: TANDEM_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: TANDEM_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: TANDEM_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: TANDEM_DAEMON_AUTO_UPDATE_INTERVAL)")
...
// Empty profile → ~/.tandem/, named profile → ~/.tandem/profiles/<name>/.
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment TANDEM_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "TANDEM_SERVER_URL", "")
	if serverURL == "" {
	...
	cfg.LaunchedBy = os.Getenv("TANDEM_LAUNCHED_BY")
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := daemon.ResolveWorkspacesRoot("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "tandem daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "tandem --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".tandem", "profiles"), nil
}
