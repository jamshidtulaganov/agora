...
var rootHelpTemplate = `Work seamlessly with Tandem from the command line.

USAGE
  tandem <command> <subcommand> [flags]
{{range .Groups}}
{{.Title}}
...
EXAMPLES
  ...
  $ tandem daemon start
  $ tandem agent list --output json

ENVIRONMENT VARIABLES
  TANDEM_SERVER_URL    Override the default server URL
  TANDEM_WORKSPACE_ID  Set the active workspace

LEARN MORE
  Use ` + "`tandem <command> <subcommand> --help`" + ` for more information about a command.
`
