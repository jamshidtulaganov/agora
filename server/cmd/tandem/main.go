...
var rootCmd = &cobra.Command{
	Use:           "tandem",
	Short:         "Tandem CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Tandem from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("tandem {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Tandem server URL (env: TANDEM_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: TANDEM_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: TANDEM_DEBUG)")
