...
// agent task (TANDEM_AGENT_ID / TANDEM_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.tandem/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		t.Setenv("TANDEM_AGENT_ID", "")
		t.Setenv("TANDEM_TASK_ID", "")
		t.Setenv("TANDEM_WORKSPACE_ID", "")
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("TANDEM_AGENT_ID", "agent-123")
		t.Setenv("TANDEM_TASK_ID", "task-456")
		t.Setenv("TANDEM_WORKSPACE_ID", "env-ws")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("TANDEM_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("TANDEM_WORKSPACE_ID", "test-ws")
	t.Setenv("TANDEM_TOKEN", "test-token")
	t.Setenv("TANDEM_AGENT_ID", "")
	t.Setenv("TANDEM_TASK_ID", "")
	...
	msg := err.Error()
	if !strings.Contains(msg, "tandem agent env set") {
		t.Fatalf("no-fields error must direct users to `tandem agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `tandem agent env set` instead", flag)
		}
	}
}
...
func TestAgentSkillsAddCallsAdditiveEndpoint(t *testing.T) {
	...
	t.Setenv("TANDEM_SERVER_URL", srv.URL)
	t.Setenv("TANDEM_WORKSPACE_ID", "ws-1")
	t.Setenv("TANDEM_TOKEN", "test-token")
