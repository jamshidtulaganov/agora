...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers TANDEM_APP_URL", func(t *testing.T) {
		t.Setenv("TANDEM_APP_URL", "http://localhost:14000")
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `tandem login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
...
// `tandem login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Tandem Cloud Node PATs (`mcn_`) at
