...
import (
	...
	"github.com/jamshidtulaganov/agora/server/internal/storage"
...
// AGORA_TRUSTED_PROXIES env var. Invalid entries are dropped with a single
...
func parseTrustedProxies(raw string) []netip.Prefix {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil {
			slog.Warn("AGORA_TRUSTED_PROXIES: ignoring invalid CIDR",
				"value", s, "error", err)
			continue
		}
		out = append(out, p)
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		...
		CloudRuntimeFleetTimeout: envDuration("AGORA_CLOUD_FLEET_TIMEOUT", 35*time.Second),
	...
	if opts.BusinessMetrics != nil {
		...
		// agora_cloudruntime_request_* histograms.
	...
	// Lark integration. Only wired when AGORA_LARK_SECRET_KEY is set:
	...
	if larkKey, err := secretbox.LoadKey("AGORA_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// AGORA_LARK_SECRET_KEY is the operator's opt-in for
				...
				// real Lark traffic can point AGORA_LARK_HTTP_BASE_URL
				// at a mock server.
				//
				// AGORA_LARK_HTTP_BASE_URL is an OPTIONAL deployment-wide
				...
				larkClient := lark.NewHTTPAPIClient(lark.HTTPClientConfig{
					BaseURL: strings.TrimSpace(os.Getenv("AGORA_LARK_HTTP_BASE_URL")),
					Logger:  slog.Default(),
				})
				...
				// to initialize (bad AGORA_LARK_CALLBACK_BASE_URL or
				...
				// AGORA_LARK_REGISTRATION_DOMAIN / _LARK_DOMAIN env
				// vars override the protocol hosts for staging / dev.
				regCfg := lark.RegistrationConfig{
					Domain:     strings.TrimSpace(os.Getenv("AGORA_LARK_REGISTRATION_DOMAIN")),
					LarkDomain: strings.TrimSpace(os.Getenv("AGORA_LARK_REGISTRATION_LARK_DOMAIN")),
				}
		...
		slog.Info("lark integration disabled (AGORA_LARK_SECRET_KEY not set)")
	}
	...
	// Cloud PAT verifier: validates mcn_ tokens against Agora Cloud
	...
	// Reuses AGORA_CLOUD_FLEET_URL (the same URL the cloud-runtime
	...
	// Share the same trusted-proxy CIDRs (AGORA_TRUSTED_PROXIES) so the
	...
	r.With(authRL).Get("/auth/invitations/{id}", h.GetInvitationAuthInfo)
	...
	// Bitrix24 inbound task webhook (no Agora auth — an optional shared
	...
	// GitHub App webhook (no Agora auth — requests are authenticated via
	...
	// Telegram bot webhook (no Agora auth). Authenticated only by the
	...
	r.Group(func(r chi.Router) {
		...
		r.Post("/api/zoho-projects/welcome-emails", h.SendZohoWelcomeEmails)
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					r.Get("/actor-directory", h.ListActorDirectory)
				...
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceRoleFromURL(queries, "id", "owner", "admin"))
					r.With(handler.RequireHumanActor).Post("/telegram/test-fixtures/delete", h.DeleteTelegramFixtureMessages)
				})
				r.Group(func(r chi.Router) {
					r.Use(middleware.RequireWorkspaceRoleFromURL(queries, "id", "owner", "admin"))
		...
		// cloud-runtime — agora-cloud's Fleet and Billing share
		...
		r.Group(func(r chi.Router) {
			...
			// Fine-tuning dataset export (run traces -> training examples)
			r.Get("/api/datasets/runs", h.ExportRunDataset)
			...
			r.Route("/api/agents", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					// Per-agent Telegram bot, so the agent can speak in a group
					// under its own identity (see telegram_installation.go).
					r.Get("/telegram", h.GetAgentTelegramBot)
...
// AGORA_LARK_CALLBACK_BASE_URL), we log and fall back to the
...
func cloudRuntimeFleetURLFromEnv() string {
	if url := strings.TrimSpace(os.Getenv("AGORA_CLOUD_FLEET_URL")); url != "" {
		return url
	}
	return strings.TrimSpace(os.Getenv("AGORA_FLEET_URL"))
}
