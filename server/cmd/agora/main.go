...
var rootCmd = &cobra.Command{
	Use:           "agora",
	Short:         "Agora CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Agora from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("agora {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Agora server URL (env: AGORA_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: AGORA_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: AGORA_DEBUG)")
