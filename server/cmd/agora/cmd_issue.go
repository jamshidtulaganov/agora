...
func init() {
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Agora-Next-Before response header; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Agora-Next-Before-Id response header; must be paired with --before.")
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("AGORA_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse AGORA_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	// Quick-create stamp: when the daemon sets AGORA_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `agora issue create` call
	...
	if taskID := os.Getenv("AGORA_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-Agora-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Agora-Next-Before-Id"); nbid != "" {
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set AGORA_WORKSPACE_ID")
	}
...
// from `agora workspace member list --output json`) should use this instead of
