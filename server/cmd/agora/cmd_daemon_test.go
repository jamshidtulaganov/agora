...
func TestPrintDiskUsageEmptyHintSuggestsProfilesWithTasks(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("AGORA_WORKSPACES_ROOT", "")
	...
	printDiskUsageEmptyHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "agora_workspaces"),
	}, "", "")
	...
	if !strings.Contains(got, "agora --profile two-tasks daemon disk-usage") {
		t.Fatalf("hint output = %q, want two-tasks profile command", got)
	}
	if !strings.Contains(got, "agora --profile one-task daemon disk-usage") {
		t.Fatalf("hint output = %q, want one-task profile command", got)
	}
	if !strings.Contains(got, "agora --profile 'space profile' daemon disk-usage") {
...
func TestPrintDiskUsageEmptyHintSuggestsDefaultFromNamedProfile(t *testing.T) {
	...
	var out bytes.Buffer
	printDiskUsageEmptyHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "agora_workspaces_named"),
	}, "named", "")

	got := out.String()
	if !strings.Contains(got, "agora daemon disk-usage") {
...
func writeDiskUsageTaskFile(t *testing.T, home, profile, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "agora_workspaces_"+profile, workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}

func writeDefaultDiskUsageTaskFile(t *testing.T, home, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "agora_workspaces", workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}
