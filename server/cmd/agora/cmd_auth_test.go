...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers AGORA_APP_URL", func(t *testing.T) {
		t.Setenv("AGORA_APP_URL", "http://localhost:14000")
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `agora login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
...
// `agora login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Agora Cloud Node PATs (`mcn_`) at
