...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Tandem Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Tandem user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, tandem_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Tandem user is removed from the workspace the stale Lark
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Tandem `chat_session`. The Lark side
...
-- =====================
-- Maps a Tandem task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Tandem user. The
