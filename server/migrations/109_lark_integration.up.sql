...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Agora Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Agora user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, agora_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Agora user is removed from the workspace the stale Lark
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Agora `chat_session`. The Lark side
...
-- =====================
-- Maps a Agora task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Agora user. The
