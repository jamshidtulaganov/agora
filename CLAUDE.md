...
Agora is an AI-native task management platform — like Linear, but with AI agents as first-class citizens.
...
- **Mobile (`apps/mobile/`) is independent.** It shares only **types and pure functions** from `@agora/core/`, with `import type` for types (zero runtime coupling). UI, state, hooks, providers, i18n, React version, build pipeline, release cadence — all mobile-owned.
...
make cli ARGS="..."   # Run agora CLI (e.g. make cli ARGS="config")
make test             # Go tests
...
pnpm --filter @agora/views exec vitest run auth/login-page.test.tsx
pnpm --filter @agora/core exec vitest run runtimes/version.test.ts
pnpm --filter @agora/web exec vitest run app/\(auth\)/login/page.test.tsx
...
pnpm --filter @agora/desktop build      # Compile TS → JS (reads .env.production)
pnpm --filter @agora/desktop package    # Package into .app/.dmg/.exe (current platform only)
...
- `packages/ui/` — zero `@agora/core` imports (pure UI, no business logic).
...
Rules for `apps/mobile/` live in `apps/mobile/CLAUDE.md`. Read it before touching anything in `apps/mobile/` — it covers what may be imported from `@agora/core/`, the React version policy, the build/release pipeline, and the locked tech-stack baseline.
...
Every full-window desktop view (anything outside the dashboard shell) must mount `<DragStrip />` from `@agora/views/platform` as the first flex child of the page root, otherwise users can't drag the window. Interactive UI inside the top 48px needs `WebkitAppRegion: "no-drag"` to stay clickable.
...
**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` or `react-router-dom` to test a component from `@agora/views`, the test is in the wrong place — move it to `packages/views/` and mock `@agora/core` instead.
...
- Mock `@agora/core` stores with `vi.hoisted()` + `Object.assign(selectorFn, { getState })` pattern (Zustand stores are both callable and have `.getState()`).
- Mock `@agora/core/api` for API calls.
