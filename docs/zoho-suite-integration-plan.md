...
1. **CRM: which modules count as issues?** ✅ **DECIDED — CRM = Tasks only.** Support lives in **Zoho Desk** (the customer uses Desk; CRM Cases is the pre-Desk support module and is unused here), so CRM Cases are NOT synced — support tickets come exclusively from the Desk channel (no double-import). Deals/Leads are linked context only. *(Original answer was "Tasks + Cases"; revised to Tasks-only after confirming Desk is the support source.)*
2. **Desk: departments → projects.** ✅ **DECIDED — per-department routing:** each Desk department → its own Agora project inside Octane, via `ZOHO_DESK_DEPARTMENT_MAP` (mirrors `BITRIX_GROUP_MAP`). Still open: *which* departments to sync (the `departmentIds` filter bounds volume + Zoho credit cost) — default all, narrow later if noisy.
...
4. **Realtime vs poll.** ✅ **DECIDED — realtime required:** P5 webhooks are IN scope (Desk Ticket Add/Update rules + CRM watch channels), with the poll loop kept as the reconciliation backstop (webhooks are best-effort + capped + CRM channels expire ≤1 week, so poll must always run underneath). Not poll-only.
